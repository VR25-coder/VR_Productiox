(function() {
  ...
  function buildPortfolioForm(data) {
    ...
    // Helper: upload project media (thumbnails / videos) and get URLs to paste into the Projects field
    const projectMediaHelper = document.createElement('div');
    projectMediaHelper.className = 'field-group';
    projectMediaHelper.innerHTML = `
      <label>Project media upload helper</label>
      <p style="font-size:0.8rem;color:#aaa;margin-bottom:0.5rem;">
        Upload a thumbnail image or video file here. The server will return a public URL that you can paste into the
        <strong>Projects</strong> textarea (use it in the <code>link-or-file</code> or <code>thumbnail</code> column).
      </p>
      <div style="display:flex;flex-direction:column;gap:0.75rem;">
        <div>
          <span style="font-size:0.8rem;">Upload thumbnail image</span>
          <input id="pf_upload_project_thumbnail_generic" type="file" accept="image/*" style="margin-top:0.25rem;">
          <input id="pf_output_project_thumbnail_generic" type="text" readonly
                 style="margin-top:0.25rem;width:100%;padding:0.4rem 0.6rem;border-radius:6px;border:1px solid #333;background:#000;color:#fff;font-size:0.8rem;"
                 placeholder="Thumbnail URL will appear here">
        </div>
        <div>
          <span style="font-size:0.8rem;">Upload video file</span>
          <input id="pf_upload_project_video_generic" type="file" accept="video/*" style="margin-top:0.25rem;">
          <input id="pf_output_project_video_generic" type="text" readonly
                 style="margin-top:0.25rem;width:100%;padding:0.4rem 0.6rem;border-radius:6px;border:1px solid #333;background:#000;color:#fff;font-size:0.8rem;"
                 placeholder="Video URL will appear here">
        </div>
      </div>
    `;
    portfolioFormEl.appendChild(projectMediaHelper);

    const thumbUpload = projectMediaHelper.querySelector('#pf_upload_project_thumbnail_generic');
    const thumbOutput = projectMediaHelper.querySelector('#pf_output_project_thumbnail_generic');
    if (thumbUpload && thumbOutput) {
      thumbUpload.addEventListener('change', () => {
        const file = thumbUpload.files && thumbUpload.files[0];
        if (!file) return;
        thumbUpload.disabled = true;
        uploadFileToServer(file, 'images')
          .then(info => {
            const url = (info && (info.url || info.fileName)) || '';
            if (!url) return;
            thumbOutput.value = url;
            if (navigator.clipboard && navigator.clipboard.writeText) {
              navigator.clipboard.writeText(url).catch(() => {});
            }
            alert('Thumbnail uploaded. URL copied to clipboard. Paste it into the "Projects" field (thumbnail column).');
          })
          .catch(() => {
          ...
          .finally(() => {
            thumbUpload.value = '';
            thumbUpload.disabled = false;
          });
      });
    }

    const videoUpload = projectMediaHelper.querySelector('#pf_upload_project_video_generic');
    const videoOutput = projectMediaHelper.querySelector('#pf_output_project_video_generic');
    if (videoUpload && videoOutput) {
      videoUpload.addEventListener('change', () => {
        ...
        uploadFileToServer(file, 'videos')
          .then(info => {
            const url = (info && (info.url || info.fileName)) || '';
            if (!url) return;
            videoOutput.value = url;
            if (navigator.clipboard && navigator.clipboard.writeText) {
              navigator.clipboard.writeText(url).catch(() => {});
            }
            alert('Video uploaded. URL copied to clipboard. Paste it into the "Projects" field (link-or-file column and set type=video).');
          })
          .catch(() => {
